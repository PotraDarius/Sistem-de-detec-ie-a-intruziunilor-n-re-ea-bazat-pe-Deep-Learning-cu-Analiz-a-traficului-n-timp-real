...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-zinc-950 text-zinc-200 font-sans relative overflow-hidden flex items-center justify-center p-4">
      <style>{`
      ...
      <div className="relative z-10 w-full max-w-md">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="font-mono text-base tracking-[0.18em] uppercase text-zinc-100 font-medium">
                NIDS <span className="text-cyan-400">/</span> Sentinel
              </h1>
        ...
        {/* Card */}
        <div className="rounded border border-zinc-800/80 bg-zinc-950/70 backdrop-blur-sm overflow-hidden">
          ...
          <form onSubmit={onSubmit} className="p-5 space-y-4">
            ...
            <div className="text-center font-mono text-[10px] text-zinc-600 uppercase tracking-wider pt-1">
              {mode === 'login' ? (
                <>nu ai cont? <button type="button" className="text-cyan-400 hover:text-cyan-300" onClick={() => { setMode('register'); setError(null); }}>înregistrează-te</button></>
              ) : (
                <>ai deja cont? <button type="button" className="text-cyan-400 hover:text-cyan-300" onClick={() => { setMode('login'); setError(null); }}>autentifică-te</button></>
              )}
            </div>
          </form>
        </div>

        <div className="mt-4 text-center font-mono text-[10px] text-zinc-600 uppercase tracking-wider">
          nids · 1d-cnn
        </div>
      </div>
    </div>
